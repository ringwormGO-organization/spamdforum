	...
	if (isset($_POST['register'])) {
		...
		if ($name && $email && $passwd) {
			if (check_exist_email($email, $dbc)) {
				include "{$_SERVER['DOCUMENT_ROOT']}/html/header.html";
				$msg .=
				$registerphp['msg']['err_email_existed'];
				include "{$_SERVER['DOCUMENT_ROOT']}/html/footer.html";
				exit;
			}
...
<table style="text-align: center; margin-left:auto; margin-right:auto; 
	...
	<tr>
		...
		<td><label for="name"><input id="name" type="text" name="name" size="64" maxlength="127" value="<?php if(isset($_POST['name'])) {echo export_data($_POST['name']);} ?>"></label></td>
	</tr>
	<tr>	
		<th style="width:30%;"><?php echo $registerphp['form_input']['email']; ?>:</th> 
		<td><label for="email"><input id="email" type="text" name="email" size="64" maxlength="127" value="<?php if(isset($_POST['email'])) {echo export_data($_POST['email']);} ?>"></label></td>
	</tr>
	<tr>
